...
import { classMasteryRadar, demoAccounts } from "../data/mockData";
...
export default function LoginPage({ onLogin }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (!email.trim() || !password.trim()) {
      setError("Enter both an email and a password, or click a demo account below.");
      return;
    }

    const match = Object.entries(demoAccounts).find(
      ([, acct]) => acct.email.toLowerCase() === email.trim().toLowerCase() && acct.password === password
    );
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: "44px 40px", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ marginTop: 22, padding: "12px 14px", background: COLORS.panelAlt, border: `1px solid ${COLORS.border}`, borderRadius: 10 }}>
            <div style={{ fontFamily: "Inter", fontSize: 11, fontWeight: 700, color: COLORS.sub, letterSpacing: 0.4, marginBottom: 10 }}>DEMO ACCOUNTS</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {Object.entries(demoAccounts).map(([r, acct]) => (
                <button key={r} type="button" onClick={() => handleFill(r)} style={{
                  display: "flex", alignItems: "center", gap: 10, background: COLORS.panel,
                  border: `1px solid ${COLORS.border}`, borderRadius: 9, padding: "9px 11px",
                  cursor: "pointer", textAlign: "left", width: "100%",
                }}>
                  {r === "admin" ? <GradCapIcon size={14} color={COLORS.teal} /> : <ShieldCheck size={14} color={COLORS.amber} />}
                  <div style={{ flex: 1 }}>
                    <div style={{ fontFamily: "Inter", fontSize: 11.5, fontWeight: 600, color: COLORS.text }}>{acct.roleLabel}</div>
                    <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10.5, color: COLORS.sub }}>{acct.email}</div>
                  </div>
                  <ChevronRight size={13} color={COLORS.sub} />
                </button>
              ))}
            </div>
            <div style={{ fontFamily: "Inter", fontSize: 10.5, color: COLORS.sub, marginTop: 8 }}>Click an account to autofill, then press Sign in.</div>
          </div>
        </div>
      </div>
    </div>
