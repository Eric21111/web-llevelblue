...
export default function TeacherHome() {
  ...
  const avgMastery = classMasteryRadar.length > 0 ? Math.round(classMasteryRadar.reduce((a, b) => a + b.mastery, 0) / classMasteryRadar.length) : 0;
  const avgGain = students.length > 0 ? Math.round(students.reduce((a, s) => a + (s.post - s.pre), 0) / students.length) : 0;
  const activeRate = students.length > 0 ? Math.round((students.filter(s => s.sessions > 0).length / students.length) * 100) : 0;
  ...
  return (
    <>
      <div style={{ display: "flex", gap: 16, marginBottom: 20, flexWrap: "wrap" }}>
        ...
        <StatCard icon={Activity} label="Weekly Active Rate" value={activeRate} suffix="%" delta="3%" deltaUp accent={COLORS.amber} />
      </div>
